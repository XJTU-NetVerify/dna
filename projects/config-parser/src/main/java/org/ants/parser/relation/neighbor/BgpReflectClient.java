...
public class BgpReflectClient extends Neighbor implements Relation {
  public BgpReflectClient(Node node1, String ip1, Node node2, String ip2) {
    super(node1, ip1, node2, ip2);
  }

  @Override public String toString() {
    return "BgpReflectClient"+super.toString();
  }
  ...
  @Override public void insertRecord(routingUpdateBuilder builder) {
    ...
    builder.insert_BgpReflectClient(node1.DType(builder), ip1_w, node2.DType(builder), ip2_w);
  }

  @Override public void deleteRecord(routingUpdateBuilder builder) {
    ip_tWriter ip1_w = RelationWrapper.ipWrapper(ip1, builder);
    ip_tWriter ip2_w = RelationWrapper.ipWrapper(ip2, builder);
    builder.delete_BgpReflectClient(node1.DType(builder), ip1_w, node2.DType(builder), ip2_w);
  }
}
